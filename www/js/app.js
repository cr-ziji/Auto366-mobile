class Auto366App {
    ...
    _initEventListeners() {
        ...
        if (openWebsiteBtn) {
            openWebsiteBtn.addEventListener('click', () => {
                if (window.FlipbookScanner && FlipbookScanner.openUrl) {
                    FlipbookScanner.openUrl('https://366.cyril.qzz.io');
                } else {
    ...
    /** 把监听/自动画中画状态同步给原生层（上划手势自动进入画中画的判定依据） */
    _syncNativeAppState() {
        if (window.FlipbookScanner && typeof FlipbookScanner.setAppState === 'function') {
            FlipbookScanner.setAppState({
                monitoring: this.isMonitoring === true,
                autoPip: this.settings.autoPipMode === true
            });
        }
    }
    ...
    async _pollFlipbook() {
        ...
        try {
            // 快速路径：一次 listTree 原生调用返回整棵子树，
            // 替代原来"每个子目录一次 listFiles + 递归求大小"的 N 次桥接往返
            if (window.FlipbookScanner && typeof FlipbookScanner.listTree === 'function') {
                const root = await new Promise((resolve) => {
                    FlipbookScanner.listTree(this.flipbookDirPath, 12, (node) => resolve(node), (err) => {
                        this.addLog('读取目录失败: ' + err, 'error');
                ...
                for (const child of (root.children || [])) {
                    ...
                    const currentSize = Auto366App.computeTreeSize(child);
                    newDirSizes[child.name] = currentSize;

                    const prevSize = this.dirSizes[child.name];
                    if (prevSize === undefined) {
                        this.knownFiles.add(child.name);
                        this.addLog('发现新目录: ' + child.name, 'info');
                        this._scanDirFiles(child, []);
                    } else if (currentSize > prevSize) {
                        this.addLog('目录 ' + child.name + ' 大小变化: ' + prevSize + ' -> ' + currentSize, 'info');
                        this.scannedDirs.delete(child.name);
                        this._scanDirFiles(child, []);
                    }
                }
            ...
            for (const entry of entries) {
                ...
                fallbackSizes[name] = currentSize;
            ...
            this.dirSizes = fallbackSizes;
        } catch (error) {
            console.error('Poll error:', error);
        }
    }

    /** 递归统计目录树内所有文件的大小（与原 _getDirTotalSize 语义一致） */
    static computeTreeSize(node) {
        let total = 0;
        const children = node.children || [];
        for (const child of children) {
            if (child.isDirectory) {
                total += Auto366App.computeTreeSize(child);
            } else {
                total += child.size || 0;
            }
        }
    ...
    /** 简易时间戳防抖：waitMs 内的重复调用被忽略，返回是否放行 */
    _debounceAction(key, waitMs) {
        if (!this._pipDebounceTs) this._pipDebounceTs = {};
        const now = Date.now();
        if (now - (this._pipDebounceTs[key] || 0) < waitMs) return false;
        this._pipDebounceTs[key] = now;
        return true;
    }

    /**
     * 应用小窗 UI。force=true 用于系统回调驱动（必须生效），
     * false 用于手动按钮等可被防抖拦截的场景。
     */
    _enterPipMode(force) {
        if (this._pipActive) return;
        if (!force && !this._debounceAction('pipEnter', 250)) return;
    ...
    _exitPipMode(force) {
        if (!this._pipActive && !force) return;
        if (!this._debounceAction('pipExit', 250)) return;

        this._pipActive = false;
        this._clearPipConfirmTimer();
    ...
    _requestPipMode() {
        if (!window.FlipbookScanner) return;
        // 防抖：600ms 内重复点击/触发只请求一次原生画中画
        if (!this._debounceAction('pipRequest', 600)) return;
        var self = this;
        this._clearPipConfirmTimer();
        // 原生调用是"发射后不管"：系统可能拒绝进入（熄屏、多任务太晚等）。
        // 若 1.5s 内没有收到 onPictureInPictureModeChanged(true) 的真实确认，
        // 说明进入失败，回滚小窗 UI。
        this._pipConfirmTimer = setTimeout(function () {
            self._pipConfirmTimer = null;
            if (self._pipActive && !self._pipConfirmed) {
                self.addLog('画中画进入失败，已恢复界面', 'warning');
                self._exitPipMode(true);
            }
        }, 1500);
        FlipbookScanner.enterPipMode(function () {}, function () {});
    }

    _clearPipConfirmTimer() {
        if (this._pipConfirmTimer) {
            clearTimeout(this._pipConfirmTimer);
            this._pipConfirmTimer = null;
        }
    }
    ...
    _onPipModeChanged(isInPip) {
        // 尾随防抖：短时间内的多次模式切换事件只应用最终状态
        this._pendingPipState = isInPip;
        if (this._pipChangeTimer) {
            clearTimeout(this._pipChangeTimer);
        }
        this._pipChangeTimer = setTimeout(() => {
            this._pipChangeTimer = null;
            const inPip = this._pendingPipState === true;
            if (inPip) {
                // 系统真实确认进入画中画
                this._pipConfirmed = true;
                this.addLog('已进入浮窗模式', 'info');
                this._enterPipMode(true);
            } else {
                this.addLog('已退出浮窗模式', 'info');
                this._exitPipMode(true);
            }
        }, 150);
    }
